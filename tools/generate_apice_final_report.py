...
def final_output_row(rec: dict[str, str], feature: dict[str, str], changes: list[dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "impression_share": rounded(rec.get("impression_share")),
        "lost_is_budget": rounded(rec.get("lost_is_budget")),
        "lost_is_rank": rounded(rec.get("lost_is_rank")),
...
def recommendation_block(row: dict[str, Any]) -> list[str]:
    return [
        ...
        f"Impression share: {row['impression_share']}",
        f"Lost IS budget: {row['lost_is_budget']}",
        f"Lost IS rank: {row['lost_is_rank']}",
